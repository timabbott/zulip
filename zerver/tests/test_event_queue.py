...
class MissedMessageNotificationsTest(ZulipTestCase):
    ...
    def test_enqueue_notifications(self) -> None:
        ...
        email_notice, mobile_notice = self.check_will_notify(
            ...
            mentioned=False, wildcard_mention_email_notify=False, wildcard_mention_push_notify=False,
        ...
        email_notice, mobile_notice = self.check_will_notify(
            user_profile.id, message_id, private_message=False,
            mentioned=True, wildcard_mention_email_notify=False, wildcard_mention_push_notify=False,
            stream_push_notify=False, stream_email_notify=False,
            stream_name=None, always_push_notify=False, idle=True, already_notified={})
        self.assertTrue(email_notice is not None)
        self.assertTrue(mobile_notice is not None)

        # Wildcard mention sends a email notice with email configured
        email_notice, mobile_notice = self.check_will_notify(
            user_profile.id, message_id, private_message=False,
            mentioned=False, wildcard_mention_email_notify=True, wildcard_mention_push_notify=False,
            stream_push_notify=False, stream_email_notify=False,
            stream_name=None, always_push_notify=False, idle=True, already_notified={})
        self.assertTrue(email_notice is not None)
        self.assertTrue(mobile_notice is None)

        # Wildcard mention sends a push notice with push configured
        email_notice, mobile_notice = self.check_will_notify(
            user_profile.id, message_id, private_message=False,
            mentioned=False, wildcard_mention_email_notify=False, wildcard_mention_push_notify=True,
            stream_push_notify=False, stream_email_notify=False,
            stream_name=None, always_push_notify=False, idle=True, already_notified={})
        self.assertTrue(email_notice is None)
        self.assertTrue(mobile_notice is not None)
        ...
        email_notice, mobile_notice = self.check_will_notify(
            user_profile.id, message_id, private_message=False,
            mentioned=False, wildcard_mention_email_notify=True, wildcard_mention_push_notify=True,
            stream_push_notify=False, stream_email_notify=False,
            stream_name=None, always_push_notify=False, idle=False, already_notified={})
        self.assertTrue(email_notice is None)
        self.assertTrue(mobile_notice is None)
    ...
    def test_end_to_end_missedmessage_hook(self) -> None:
        ...
        with mock.patch("zerver.tornado.event_queue.maybe_enqueue_notifications") as mock_enqueue:
            ...
            self.assertEqual(args_list, (user_profile.id, msg_id, False, False, False, False, False, False,
                                         "Denmark", False, True,
        ...
        with mock.patch("zerver.tornado.event_queue.maybe_enqueue_notifications") as mock_enqueue:
            ...
            self.assertEqual(args_list, (user_profile.id, msg_id, True, False, False, False, False,
        ...
        with mock.patch("zerver.tornado.event_queue.maybe_enqueue_notifications") as mock_enqueue:
            ...
            self.assertEqual(args_list, (user_profile.id, msg_id, False, True, False, False, False,
        ...
        with mock.patch("zerver.tornado.event_queue.maybe_enqueue_notifications") as mock_enqueue:
            ...
            self.assertEqual(args_list, (user_profile.id, msg_id, False, False, True, True, False,
                                         False, "Denmark", False, True,
                                         {'email_notified': True, 'push_notified': True}))
        destroy_event_queue(client_descriptor.event_queue.id)

        # TODO: Add additional tests with changes in wildcard_mention_email_notify / wildcard_mention_push_notify.

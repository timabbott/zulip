...
def missedmessage_hook(user_profile_id: int, client: ClientDescriptor, last_for_client: bool) -> None:
    ...
    for event in client.event_queue.contents():
        ...
        wildcard_mention_push_notify = event.get('wildcard_mention_push_notify', False)
        wildcard_mention_email_notify = event.get('wildcard_mention_email_notify', False)
        ...
        maybe_enqueue_notifications(user_profile_id, message_id, private_message, mentioned,
                                    wildcard_mention_email_notify, wildcard_mention_push_notify,
...
def maybe_enqueue_notifications(user_profile_id: int, message_id: int, private_message: bool,
    ...
    if (idle or always_push_notify) and (private_message or mentioned or
                                         wildcard_mention_push_notify or stream_push_notify):
        notice = build_offline_notification(user_profile_id, message_id)
        if private_message:
            notice['trigger'] = 'private_message'
        elif mentioned:
            notice['trigger'] = 'mentioned'
        elif wildcard_mention_push_notify:
    ...
    # above.
    if idle and (private_message or mentioned or wildcard_mention_email_notify or stream_email_notify):
        notice = build_offline_notification(user_profile_id, message_id)
        if private_message:
            notice['trigger'] = 'private_message'
        elif mentioned:
            notice['trigger'] = 'mentioned'
        elif wildcard_mention_email_notify:
            notice['trigger'] = 'wildcard_mentioned'
...
def process_message_event(event_template: Mapping[str, Any], users: Iterable[Mapping[str, Any]]) -> None:
    ...
    for user_data in users:
        ...
        # These computed values are whether the user is both configured
        # to get that type of notification for wildcard mentions and
        # also the (non-muted) message contained a wildcard mention.
        wildcard_mention_email_notify = (
            wildcard_mentioned and user_data.get('wildcard_mention_email_notify', False)
        )
        wildcard_mention_push_notify = (
            wildcard_mentioned and user_data.get('wildcard_mention_push_notify', False)
        )

        # We first check if a message is potentially mentionable,
        # since receiver_is_off_zulip is somewhat expensive.
        if (private_message or mentioned or wildcard_mention_email_notify
                or wildcard_mention_push_notify or stream_push_notify or stream_email_notify):
            ...
            result['wildcard_mention_push_notify'] = wildcard_mention_push_notify
            result['wildcard_mention_email_notify'] = wildcard_mention_email_notify
...
def process_message_update_event(event_template: Mapping[str, Any],
    ...
    wildcard_mention_push_user_ids = set(event_template.get('wildcard_mention_push_user_ids', []))
    wildcard_mention_email_user_ids = set(event_template.get('wildcard_mention_email_user_ids', []))
    ...
    for user_data in users:
        ...
        wildcard_mention_email_notify = wildcard_mentioned and (
            user_profile_id in wildcard_mention_email_user_ids)
        wildcard_mention_push_notify = wildcard_mentioned and (
            user_profile_id in wildcard_mention_push_user_ids)

...
* The Tornado [event queue system](../subsystems/events-system.md)
  ...
  * We solve that problem by also notifying if
    `receiver_is_off_zulip`, which checks whether the user has any
  ...
  * The `receiver_is_off_zulip` check is effectively repeated when
    event queues are garbage-collected (in `missedmessage_hook`);
    this second check solves the Hard Disconnect Problem, resulting in
...
* The queue processors for those queues make the final determination
  ...
  important to know that it's normal for a message may sit in these
...
* Both queue processor code paths do additional filtering before
  ...
  * The **Email notifications queue processor**, `MissedMessageWorker`,
  takes care to delay a 2 minutes (hopefully in the future, a
  configuration setting) and batch together multiple messages in a
  single thread into a single email.  These features are unnecessary
  ...
  * The **Push notifications queue processor**,
  ...
  notification. This logic is somewhat complicated by logic tracking
...
* **Future configuration**. Notification settings are an area that we
  ...
  that topic in a stream). Generally the goal is for logic around
* **Message editing**. Zulip supports editing messages, and that
  interacts with notifications carefully: Notifications should have

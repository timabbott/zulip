...
export function initialize() {
    popover_menus.register_popover_menu("#personal-menu", {
        ...
        onMount(instance) {
            ...
            $popper.one("click", ".personal-menu-actions .narrow_to_private_messages", (e) => {
                const user_id = elem_to_user_id($(e.target).closest(".personal-menu-actions"));
                const email = people.get_by_user_id(user_id).email;
                popovers.hide_all();
                narrow.by("dm", email, {trigger: "personal menu popover"});
                e.preventDefault();
            });

            $popper.one("click", ".personal-menu-actions .narrow_to_messages_sent", (e) => {
                const user_id = elem_to_user_id($(e.target).closest(".personal-menu-actions"));
                const email = people.get_by_user_id(user_id).email;
                popovers.hide_all();
                narrow.by("sender", email, {trigger: "personal menu popover"});
                e.preventDefault();
            });
